...
<script>
    ...
    ?>
        ...
        if ($rest["id"] == $_SESSION["pd_aberto"]) {

            $sku = str_pad($rest['sku'] , 3 , '0' , STR_PAD_LEFT);
        ?>
            pdEditavel["titulo"] = '<?php echo $rest["nome"]; ?>';
            pdEditavel["tabela_json"] = <?php echo $rest["tabelas_json"]; ?>;
            pdEditavel["sku"] = '<?php echo $sku; ?>';
            pdEditavel["custo"] = <?php echo $rest["preco_compra"]; ?>;
            pdEditavel["venda"] = <?php echo $rest["preco_revenda"]; ?>;
            pdEditavel["imagem"] = '<?php echo $rest["imagem"]; ?>';
            pdEditavel["categoria_id"] = <?php echo $rest["categoria_id"]; ?>;
            pdEditavel["anotacoes"] = '<?php echo $rest["anotacoes"]; ?>';
    <?php
        }
    }
...
<body>
    ...
    <div id="margem-pai">
        ...
        <form method="POST" enctype="multipart/form-data" action="salvar-avancado.php">
            <div id="linha-master">
...
<script>
    document.getElementById("titulo-pd").value = pdEditavel["titulo"];
    var novoPdJson = JSON.parse(pdEditavel["tabela_json"]);
    document.getElementById("imagem-pd").style.backgroundImage = 'url(' + pdEditavel["imagem"] + ')';
    document.getElementById("imagem-pd").style.backgroundRepeat = "no-repeat";
    document.getElementById("imagem-pd").style.backgroundSize = "contain";
    document.getElementById("preco_custo").value = pdEditavel["custo"];
    document.getElementById("preco_revenda").value = pdEditavel["venda"];
    document.getElementById("sku").value = pdEditavel["sku"];
...
<script>
    ...
    function abrirProduto() {
        ...
        for (var tab = 0; tab <= novoPdJson.length - 1; tab++) {
            ...
            quantidadesCol[tab] = [];
            localCol[tab] = [];
            for (let tam = 0; tam < novoPdJson[tab].length; tam++) {
                //
